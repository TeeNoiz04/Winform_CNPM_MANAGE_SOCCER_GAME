...
{
    [TestFixture]
    public class ImagePlayerServiceTests
    {
        ...
        private ImagePlayerService _service; // Service being tested
        ...
        public void Setup()
        {
            ...
            _service = new ImagePlayerService(_context);
        }
        ...
        {
            // Purpose: Ensure that adding a new image for a player works correctly
            // Arrange: Create a player and an image
            ...
            var image = new ImagePlayer
            {
                Url = "http://example.com/player.jpg",
                PublicId = "player123",
                PlayerId = playerId,
            ...
            Assert.That(result.Url, Is.EqualTo("http://example.com/player.jpg"), "URL should match.");
            Assert.That(result.PublicId, Is.EqualTo("player123"), "Public ID should match.");
            Assert.That(result.PlayerId, Is.EqualTo(playerId), "Player ID should match.");
            ...
            var savedImage = await _context.ImagePlayers.FindAsync(result.Id);
        ...
        public async Task CreateImageAsync_InvalidUrl_ThrowsException()
        {
            ...
            // Arrange: Create a player and an image with an empty URL
            var playerId = Guid.NewGuid();
            _context.Players.Add(new Player
            {
                Id = playerId,
        ...
        public async Task CreateImageAsync_DuplicatePublicId_ThrowsException()
        {
            ...
            // Arrange: Create a player and an existing image
            var playerId = Guid.NewGuid();
            _context.Players.Add(new Player
            {
                Id = playerId,
            ...
            _context.ImagePlayers.Add(new ImagePlayer
            {
                Id = Guid.NewGuid(),
                Url = "http://example.com/player1.jpg",
                PublicId = "player123",
                PlayerId = playerId
            });
            await _context.SaveChangesAsync();
            ...
            var image = new ImagePlayer
            {
                Url = "http://example.com/player2.jpg",
                PublicId = "player123",
                PlayerId = playerId
            };
        ...
        // Test 4: Checks if creating an image with a non-existent PlayerId throws an error
        [Test]
        public async Task CreateImageAsync_InvalidPlayerId_ThrowsException()
        {
            // Purpose: Ensure that an invalid PlayerId causes an error
            // Arrange: Create an image with a random PlayerId
            var image = new ImagePlayer
            {
                Url = "http://example.com/player.jpg",
                PublicId = "player123",
            ...
            var exception = Assert.ThrowsAsync<Exception>(
                async () => await _service.CreateImageAsync(image),
                "Should throw exception for invalid PlayerId.");
            Assert.That(exception.Message, Is.EqualTo("PlayerId không tồn tại."), "Error message should indicate invalid PlayerId.");
        }
        ...
        public async Task CreateImageAsync_NullPublicId_ThrowsException()
        {
            ...
            // Arrange: Create a player and an image with null PublicId
            var playerId = Guid.NewGuid();
            _context.Players.Add(new Player
            {
                Id = playerId,
        ...
        public async Task GetAllImagesAsync_MultipleImages_ReturnsAll()
        {
            ...
            // Arrange: Create two players and two images
            var playerId1 = Guid.NewGuid();
            var playerId2 = Guid.NewGuid();
            _context.Players.AddRange(
                new Player { Id = playerId1, Name = "Player 1", National = "USA", Position = "Midfielder", Status = "Active" },
                new Player { Id = playerId2, Name = "Player 2", National = "Brazil", Position = "Defender", Status = "Active" }
            );
            _context.ImagePlayers.AddRange(
                new ImagePlayer { Id = Guid.NewGuid(), Url = "http://example.com/player1.jpg", PublicId = "player1", PlayerId = playerId1 },
                new ImagePlayer { Id = Guid.NewGuid(), Url = "http://example.com/player2.jpg", PublicId = "player2", PlayerId = playerId2 }
            );
            await _context.SaveChangesAsync();
            ...
            Assert.That(result.Any(i => i.PublicId == "player1"), Is.True, "Should include player1 image.");
            Assert.That(result.Any(i => i.PublicId == "player2"), Is.True, "Should include player2 image.");
        }
        ...
        // Test 8: Checks if getting all images includes player details
        [Test]
        public async Task GetAllImagesAsync_IncludesPlayerDetails()
        {
            // Purpose: Ensure that retrieved images include associated player information
            // Arrange: Create a player and an image
            var playerId = Guid.NewGuid();
            _context.Players.Add(new Player
            {
            ...
            // Assert: Check that player details are included
            Assert.That(result.Count, Is.EqualTo(1), "Should return one image.");
            Assert.That(result.First().Player, Is.Not.Null, "Player details should be included.");
            Assert.That(result.First().Player.Name, Is.EqualTo("John Doe"), "Player name should match.");
        }
        ...
        // Test 12: Checks if getting an image includes player details
        [Test]
        public async Task GetImageByIdAsync_IncludesPlayerDetails()
        {
            // Purpose: Ensure that retrieving an image includes associated player information
            // Arrange: Create a player and an image
            var playerId = Guid.NewGuid();
            var imageId = Guid.NewGuid();
            _context.Players.Add(new Player
            ...
            // Assert: Check that player details are included
            Assert.That(result.Player, Is.Not.Null, "Player details should be included.");
            Assert.That(result.Player.Name, Is.EqualTo("John Doe"), "Player name should match.");
        }
        ...
        public async Task UpdateImageAsync_ValidImage_ReturnsTrue()
        {
            ...
            var updatedImage = new ImagePlayer
            {
                Id = imageId,
            ...
            var savedImage = await _context.ImagePlayers.FindAsync(imageId);
        ...
        public async Task DeleteImageAsync_ExistingId_ReturnsTrue()
        {
            ...
            var deletedImage = await _context.ImagePlayers.FindAsync(imageId);
        ...
        #region GetImagesByPlayerIdAsync Tests

        // Test 21: Checks if getting images by non-existent PlayerId throws an error
        [Test]
        public async Task GetImagesByPlayerIdAsync_NonExistentPlayerId_ThrowsException()
        {
            // Purpose: Ensure that an invalid PlayerId causes an error
            // Arrange: Use a random PlayerId
            var nonExistentPlayerId = Guid.NewGuid();

            // Act & Assert
            var exception = Assert.ThrowsAsync<Exception>(
                async () => await _service.GetImagesByPlayerIdAsync(nonExistentPlayerId),
                "Should throw exception for non-existent PlayerId.");
            Assert.That(exception.Message, Is.EqualTo("Không tìm thấy cầu thủ."), "Error message should indicate player not found.");
        }

        // Test 22: Checks if getting images by empty PlayerId throws an error
        [Test]
        public async Task GetImagesByPlayerIdAsync_EmptyPlayerId_ThrowsException()
        {
            // Purpose: Ensure that an empty PlayerId causes an error
            // Arrange: Use Guid.Empty
            var emptyPlayerId = Guid.Empty;

            // Act & Assert
            var exception = Assert.ThrowsAsync<Exception>(
                async () => await _service.GetImagesByPlayerIdAsync(emptyPlayerId),
                "Should throw exception for empty PlayerId.");
            Assert.That(exception.Message, Is.EqualTo("PlayerId không hợp lệ."), "Error message should indicate invalid PlayerId.");
        }

        // Test 23: Checks if getting images for a player with no images returns an empty list
        [Test]
        public async Task GetImagesByPlayerIdAsync_NoImages_ReturnsEmptyList()
        {
            // Purpose: Ensure that a player with no images returns an empty list
            // Arrange: Create a player with no images
            var playerId = Guid.NewGuid();
            _context.Players.Add(new Player
            {
                Id = playerId,
            ...
            // Act: Get images for the player
            var result = await _service.GetImagesByPlayerIdAsync(playerId);

            // Assert: Check that the result is empty
            Assert.That(result, Is.Empty, "Should return an empty list for player with no images.");
        }
        ...
        public async Task ExistsByPublicIdAsync_ExistingPublicId_ReturnsTrue()
        {
            ...
            var result = await _service.ExistsByPublicIdAsync("player123");
        ...
        // Test 28: Checks creating multiple images for the same player
        ...
        public async Task CreateImageAsync_VeryLongUrl_ValidatesSuccessfully()
        {
            ...
            // Arrange: Create a player and an image with a long URL
            var playerId = Guid.NewGuid();
            _context.Players.Add(new Player
            {
                Id = playerId,
